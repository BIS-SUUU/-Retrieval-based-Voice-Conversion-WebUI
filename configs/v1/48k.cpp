...
struct TrainConfig {
    ...
    int segment_size = 11520;          // تغير من 12800
    int init_lr_ratio = 1;
    int warmup_epochs = 0;
    float c_mel = 45.0f;
    float c_kl = 1.0f;
...
struct DataConfig {
    ...
    int sampling_rate = 48000;          // تغير من 40000
    int filter_length = 2048;           // نفس القيمة 2048
    int hop_length = 480;               // تغير من 400
    int win_length = 2048;              // نفس القيمة 2048
    int n_mel_channels = 128;           // تغير من 125
    float mel_fmin = 0.0f;
...
struct ModelConfig {
    ...
    std::vector<int> upsample_rates = {10, 6, 2, 2, 2};     // تغيرت (كانت [10,10,2,2])
    int upsample_initial_channel = 512;
    std::vector<int> upsample_kernel_sizes = {16, 16, 4, 4, 4}; // تغيرت (كانت [16,16,4,4])
