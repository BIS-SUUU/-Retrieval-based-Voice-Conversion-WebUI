...
struct DataConfig {
    ...
    float mel_fmin = 0.0f;
    std::optional<float> mel_fmax = std::nullopt;  // لأن القيمة null
};
...
struct ModelConfig {
    ...
    std::vector<int> upsample_rates = {10, 4, 2, 2, 2};
    int upsample_initial_channel = 512;
    std::vector<int> upsample_kernel_sizes = {16, 16, 4, 4, 4};
    bool use_spectral_norm = false;
    int gin_channels = 256;
    int spk_embed_dim = 109;
};

// الهيكل الرئيسي الجامع
struct Config {
    TrainConfig train;
    DataConfig data;
    ModelConfig model;
