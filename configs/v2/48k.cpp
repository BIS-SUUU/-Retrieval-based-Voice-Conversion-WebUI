...
struct TrainConfig {
    ...
    int segment_size = 17280;          // تغير (من 12800)
    int init_lr_ratio = 1;
    int warmup_epochs = 0;
    float c_mel = 45.0f;
    float c_kl = 1.0f;
...
struct DataConfig {
    ...
    int sampling_rate = 48000;          // نفس الملف الثالث
    int filter_length = 2048;           // نفس الملف الثالث
    int hop_length = 480;               // نفس الملف الثالث
    int win_length = 2048;              // نفس الملف الثالث
    int n_mel_channels = 128;           // نفس الملف الثالث
    float mel_fmin = 0.0f;
...
struct ModelConfig {
    ...
    std::vector<int> upsample_rates = {12, 10, 2, 2};     // تغيرت
    int upsample_initial_channel = 512;
    std::vector<int> upsample_kernel_sizes = {24, 20, 4, 4}; // تغيرت
