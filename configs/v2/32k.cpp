...
struct TrainConfig {
    ...
    int segment_size = 12800;          // رجعنا للقيمة الأولى
    int init_lr_ratio = 1;
    int warmup_epochs = 0;
    float c_mel = 45.0f;
    float c_kl = 1.0f;
...
struct DataConfig {
    ...
    int sampling_rate = 32000;          // رجعنا لـ 32000
    int filter_length = 1024;           // رجعنا لـ 1024
    int hop_length = 320;               // رجعنا لـ 320
    int win_length = 1024;              // رجعنا لـ 1024
    int n_mel_channels = 80;            // رجعنا لـ 80
    float mel_fmin = 0.0f;
...
struct ModelConfig {
    ...
    std::vector<int> upsample_rates = {10, 8, 2, 2};        // تغيرت (4 أرقام)
    int upsample_initial_channel = 512;
    std::vector<int> upsample_kernel_sizes = {20, 16, 4, 4}; // تغيرت (4 أرقام)
